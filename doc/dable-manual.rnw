...
\item \texttt{bnry.list} is a list of length 2 vectors specifying, respectively,
  ...
  that level alone what the hidden level is.
\end{itemize}

\newpage
We can inspect the default values with the
'\textbf{d}able-\textbf{p}arameter-\textbf{get}' function \texttt{dpget}:
...
Note that the guide provides an automatically generated 'survival table'
(\texttt{stab}) based on the naming of variables. Also, a default grouping of
variables is always created with the value

<<"vtab.group.name">>=
dpget("vtab.group.name")
@

\noindent for all but the time-to-event variables which are grouped as
...
The survival table has no grouping, so we can create one
...
Note that the variables \emph{not} specified by the 'variable table' gets the
default grouping and are placed at the end.

To see the usefulness of setting up such a variable table, notice that with these
...
A \emph{simple descriptive table} describes only 1 type. A table describing
\emph{all} variables will be refered to as a \emph{baseline tables} - so we will
...
function. To create the default descriptive table for the \texttt{real} type we use
...
There is a shorthand for each type.
...
have to turn these features off. We can e.g.\ opt out of using automatic row
grouping (\texttt{row.group = FALSE}) as well as using labels (\texttt{t2l =
  FALSE}) as in Table \ref{tab:datex-less}.
...
\noindent It is also possible to specify \texttt{desc} as one of "all"
(default), "first" or "last" ("none" is also accepted).
...
for the \texttt{real} type, and similar help pages exists for other types
(\texttt{catg}; \texttt{bnry}; \texttt{lcat}; \texttt{date}; \texttt{surv}) and
...
\item \texttt{desc.info}: The text string "\texttt{Numeric variable: Median (Q1-Q3)}"
\end{itemize}
...
\noindent and the default baseline tester return columns
...
do a version of it again. We reuse the variable table stored in \texttt{vt} and
we will also trim down the data so that the table becomes a bit smaller, see
Table \ref{tab:bsl-0}.
...
numbers - the standardized differences - indicated by the column name. The test
part contains different kinds of test, all of which are explained through
footnotes.
...
The baseline theme 2 takes more space, but is perhaps simpler to parse, as the
information on the description is explicitly written in the first column on
(almost) each row.
