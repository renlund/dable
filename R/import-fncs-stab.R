...
##     if(is.null(affix)) affix = dparam("surv.affix")
...
##     if(is.null(prefix)) prefix = dparam("surv.prefix")
...
surv_paste <- function(s, affix, prefix = NULL){
    ...
    prefix <- dparam("surv.prefix", prefix)
...
check_stab <- function(stab){
    ...
    if(n > 0){
        ...
        if("group" %in% names(stab)){
            ...
            stab$group <- dparam("stab.group.name")
        }
        stab
    } else {
...
surv_search_string <- function(){
    ...
    prefix = dparam("surv.prefix")
    affix = dparam("surv.affix")
...
##         group.name = dparam("stab.group.name")
##     }
